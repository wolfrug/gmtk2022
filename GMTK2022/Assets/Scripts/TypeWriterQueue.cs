...
public class TypeWriterQueue : MonoBehaviour {
    ...
    public void CreateAndPlayTypeWriterQueueFromKnot (string targetKnot) {
        string[] knotStrings = GameManager.instance.m_inkStringtableManager.CreateStringArray (targetKnot);
        // Then we set it to play on the typewriter
        if (knotStrings.Length > 0) {
            WriterAction[] newQueue = TypeWriterQueue.CreateTypeWriterQueue (knotStrings);
            SetQueue (newQueue);
            StartQueue (0);
            Debug.Log ("Starting new writer queue of length " + knotStrings.Length + " with contents starting with " + knotStrings[0]);
        } else {
